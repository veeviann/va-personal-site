...
### 1.1 Comparison of R Graphics VS ggplot
::: {.panel-tabset}
...
:::
## 2 Geometric Objects in ggplot2: geom
...
### 2.3 Histograms with `geom_histogram()`
```{r}
...
### 2.4 Kernel Density Estimate with `geom_density()`
```{r}
ggplot(data=exam_data,
       aes(x=MATHS)) +
  geom_density()
...
### 2.6 Violin plot with `geom_violin()`
...
### 2.7 Scatterplot with `geom_point()`
```{r}
ggplot(data=exam_data,
       aes(x=MATHS,
           y=ENGLISH)) +
...
## 3 Statistics functions in `ggplot2`: stat
### 3.1 Using the `stat_summary()` method
...
### 3.3 Using `geom_smooth()` to add best fit curve
...
### 4.1 Wrapping 1d sequence of panels into 2d with `facet_wrap()`
```{r}
ggplot(data=exam_data,
       aes(x=MATHS)) +
  geom_histogram(bins=20) +
...
### 4.2 Creating matrix of panels with `facet_grid()`
```{r}
...
## 5 Coordinates
### 5.1 Flipping the coordinates with `coord_flip()`
```{r}
ggplot(data=exam_data, 
       aes(x=RACE)) +
  geom_bar() +
...
### 5.2 Changing the y- and x-axis range with `coord_cartesian()`
```{r}
ggplot(data=exam_data, 
       aes(x=MATHS, 
           y=ENGLISH)) +
...
### 6.1 Examples of different themes
